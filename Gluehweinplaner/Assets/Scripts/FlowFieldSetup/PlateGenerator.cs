...
{
    public static TransferType CalculatePlatePositionsAndBaseCostMatrices(Bounds b, int plateCountX, int plateCountZ)
    {
        Vector3 size = b.size;
        ...
        for (int i = 0; i < plateCountX; i++)
        {
            for (int j = 0; j < plateCountZ; j++)
            {
                ...
                {
                    Vector3 pos = new Vector3(b.min.x + i * (tileCountX * New_GenerateMatrix.TileSizeX) + (tileCountX * New_GenerateMatrix.TileSizeX) / 2,0f, b.min.z + j * (tileCountZ * New_GenerateMatrix.TileSizeX) + (tileCountZ * New_GenerateMatrix.TileSizeX) / 2);    
                    allPlates[i, j] = new Plate(pos, New_GenerateMatrix.GenerateBaseCostMatrix(tileCountX, randTileCountZ, (int row, int column) => CheckIfPlacable((row * New_GenerateMatrix.TileSizeX) + (pos.x - ((tileCountX * New_GenerateMatrix.TileSizeX) / 2)) + (New_GenerateMatrix.TileSizeX / 2), (column * New_GenerateMatrix.TileSizeZ) + (pos.z - ((randTileCountZ * New_GenerateMatrix.TileSizeZ) / 2)) + (New_GenerateMatrix.TileSizeZ / 2)), out bool onlyObstacles, out bool noObstacles));
                    allPlates[i, j].Center = pos;   
                ...
                else
                {
                    ...
                    allPlates[i, j] = new Plate(pos, New_GenerateMatrix.GenerateBaseCostMatrix(tileCountX, tileCountZ, (int row, int column) => CheckIfPlacable((row * New_GenerateMatrix.TileSizeX) + (pos.x - ((tileCountX * New_GenerateMatrix.TileSizeX)/2)) + (New_GenerateMatrix.TileSizeX / 2), (column * New_GenerateMatrix.TileSizeZ) + (pos.z - ((tileCountZ * New_GenerateMatrix.TileSizeZ)/2)) + (New_GenerateMatrix.TileSizeZ / 2)), out bool onlyObstacles, out bool noObstacles));
                    allPlates[i, j].Center = pos;
                    allPlates[i, j].Rows = tileCountX;
                    allPlates[i, j].Columns = tileCountZ;
                    allPlates[i, j].HasNoObstacles = noObstacles;
