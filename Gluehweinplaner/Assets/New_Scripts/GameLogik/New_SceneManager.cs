...
public class New_SceneManager : MonoBehaviour
{
    ...
    private New_Plate[,]? allPlateArray = null;
    private List<New_Bude>? allBudenScripts = null;
    private New_Spawner[]? allSpawner = null;
    private New_Exit[]? allExits = null;
    private New_InactiveAgentsContainer? iac = null;
    private Heatmap? hm;

    private Dictionary<Vector3, (int[,], int[,])> goalPositionToDistanceMatrix = new Dictionary<Vector3, (int[,], int[,])>();
    private Dictionary<(Vector3, Vector3), (List<New_Plate>, Queue<Vector3>, Queue<Vector3>)> allPositionsToGoals = new Dictionary<(Vector3, Vector3), (List<New_Plate>, Queue<Vector3>, Queue<Vector3>)>();
    ...
    void Start()
    {
        ...
        allBudenScripts = GameObject.Find(budenContainerName).GetComponentsInChildren<New_Bude>().ToList();
        allSpawner = GameObject.Find(spawnerContainerName).GetComponentsInChildren<New_Spawner>();
        allExits = GameObject.Find(exitContainerName).GetComponentsInChildren<New_Exit>();
    ...
    public void ResumeSimulation() { simulating = true; foreach (New_NPC ac in alleCurrentAgents) { ac.Resume(); } CalcAllBudenWeight(); }
    public void StopSimulation() { simulating = false; foreach (New_NPC ac in alleCurrentAgents) { ac.Stop(); } }
    ...
    {
        foreach (New_NPC ac in alleCurrentAgents)
        {
            ac.SetInactive();
            iac!.AddAgent(ac);
        }
        foreach (New_Bude b in allBudenScripts!) { if (b != null) { b.Reset(); } }
        inactivePlayerCount = playerCount;
        playerCount = 0;
        agentsLostPatience = 0;
        simulating = false;
        leereStellen = new LinkedList<int>();
        alleCurrentAgents = new List<New_NPC>();
        hm.Reset();
        CalcAllBudenWeight();
    }
    ...
    public void RemoveBude(New_Bude wegBude)
    {
        ...
        if (aB != null)
        {
            ...
            foreach (BudenJSON b in aB.budenArray)
            {
                ...
                if (!Physics.CheckSphere(new Vector3(pos.x, 10, pos.z), 6.25f))
                {
                    ...
                    New_Bude bd = newObj.GetComponent<New_Bude>();
    ...
    bool debugingMoves = false;
    public bool debugTilesB = false;
    public (Queue<Vector3> ,Queue<Vector3>) HandlePathRequest(Vector3 start, Vector3 goal)
    {
        if (allPositionsToGoals.ContainsKey((start,goal))) {
            List<New_Plate> np = allPositionsToGoals[(start, goal)].Item1;
            bool changed = false;
            foreach (New_Plate p in np) {
        ...
        platesToVisit = new List<New_Plate>();
        foreach (Vector2Int pos in platePosToVisit)
        {
            platesToVisit.Add(allPlateArray![pos.x, pos.y]);
        }

        (Queue<Vector3> firstStep,Queue<Vector3> wayPoints, New_Plate? lastVisitedPlate) = New_GenerateMatrix.GeneratePath(platesToVisit, start, goal, pathDiagonal);
        ...
        else
        {
            ...
            New_Plate goalPlate = allPlateArray![arrayGoal.x, arrayGoal.y];
            do
            {
                ...
                (Queue < Vector3 > newfirstStep,Queue < Vector3> newWayPoints, New_Plate? lastPlate) = New_GenerateMatrix.GeneratePath(platesToVisit, start, goal, pathDiagonal);
    ...
    {
        // SoundFXManager.instance.PlaySoundFXClip(deleteSoundClip, transform, 1f);
        allBudenScripts!.Remove(wegBude!);
        CalcAllBudenWeight();
    }
    ...
    public Queue<New_Bude> CalcNewWeightedBuden(int goalsBeforeExit)
    {
        Queue<New_Bude> returnBuden = new Queue<New_Bude>();
        List<New_Bude> allBuden = allBudenScripts!.ToList();
        int tmpKapa = maxKapazitaet;
        for (int i = 0; i < goalsBeforeExit; i++)
        {
            int rand = UnityEngine.Random.Range(0,tmpKapa+1);
            int tmpCount = 0;
            foreach (New_Bude bude in allBuden)
            {
                if (!returnBuden.Contains(bude))
                {
                    tmpCount += bude.attraktivitaet;
                    if (tmpCount > rand) { allBuden.Remove(bude); tmpKapa-=bude.attraktivitaet ; returnBuden.Enqueue(bude); break; }
                }
            }
        }
        return returnBuden;
